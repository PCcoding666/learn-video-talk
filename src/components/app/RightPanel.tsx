...
import { Send, Sparkles, Trash2, StopCircle, User, Bot, X } from "lucide-react";
...
interface Message {
  ...
  imageUrls?: string[];  // 用户消息携带的关键帧图片（支持多张）
  imageFrameIds?: number[];  // 关键帧ID数组
}
...
const RightPanel = ({ 
  ...
  const initializeChatSession = async () => {
    ...
    try {
      // 不传递metadata，让后端自动从Supabase加载完整的转录数据和关键帧
  ...
  return (
    <aside className="w-[25%] min-w-[300px] max-w-[400px] border-l border-border bg-gradient-to-b from-background to-muted/20 flex flex-col">
      {/* Header */}
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                <Input
                  ...
                  placeholder={selectedKeyframes.length > 0 ? `输入关于这 ${selectedKeyframes.length} 张图片的问题...` : "输入你的问题... (Shift+Enter 换行)"}
